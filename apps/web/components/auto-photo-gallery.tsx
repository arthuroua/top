...
export function AutoPhotoGallery({ images, vehicleName }: AutoPhotoGalleryProps) {
  ...
  const restThumbs = images.slice(5, 13);
  const hiddenCount = Math.max(0, images.length - 13);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowRight") {
        setActiveIndex((current) => (current === null ? 0 : Math.min(images.length - 1, current + 1)));
      }
      if (event.key === "ArrowLeft") {
        setActiveIndex((current) => (current === null ? 0 : Math.max(0, current - 1)));
      }
    };
  ...
  return (
    <>
      <div className={`autoTopGallery ${images.length === 1 ? "autoTopGallerySingle" : ""}`}>
        <div className="autoTopGalleryStage">
          ...
          {restThumbs.length > 0 && (
            <div className="autoTopMoreThumbs" aria-label={`${vehicleName} more auction photos`}>
              ...
              {hiddenCount > 0 && (
                <button type="button" className="autoTopThumbCounter" onClick={() => setActiveIndex(5)}>
                  +{hiddenCount}
                </button>
              )}
            </div>
          ...
          <div className="photoLightboxPanel">
            ...
            {images.length > 1 && (
              <>
                <button
                  type="button"
                  className="photoLightboxArrow photoLightboxArrowPrev"
                ...
                <button
                  type="button"
                  className="photoLightboxArrow photoLightboxArrowNext"
                  onClick={showNext}
